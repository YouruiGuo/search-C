...
*/
Grid_env DifferentialHeuristic::getEnv() {
    return env;
}
...
void DifferentialHeuristic::dijkstraSearch(std::vector<State> *s, int id) {
    ...
    for (unsigned int i = 0; i < s->size(); i++) {
        ...
        temp = (*s)[i];
        cout << "in dijkstraSearch" << temp.getState()[0] << ' ' << temp.getState()[1] << endl;
    }
    while (!Q.empty()) {
        numExpandNode++;
        ...
        if (temp.getState()[0] > env.getMapsize()[0] ||
            temp.getState()[1] > env.getMapsize()[1]){
            cout << temp.getState()[0] << ' ' << temp.getState()[1] << endl;
        }
        Q.pop();
        if (Q.empty()) {
            lastState = temp;
...
void DifferentialHeuristic::writeToFile(std::vector<int> v) {
    std::string name = "/Users/margaret/Documents/cmput652/search-C1/asg4/data/heuristic_random.txt";
    ofstream f(name);
    int k = 0;
    if (f.is_open()) {
        for (unsigned int i = 0; i < totalnum; ++i) {
...
void DifferentialHeuristic::optimizedPlacement() {
    ...
    if (stat(name.c_str(), &buffer) == 0) {
        ...
        for (int i = 0; i < 30; ++i) {
            h.clear();
            h.push_back(t[i]);
            cout << t[i].getState()[0] << ' ' << t[i].getState()[1]  << endl;
            dijkstraSearch(&h, i);
            cout << "heuristic" << i << "built" << endl;
        }
